...
// ── GRAFIEK BEREIK INSTELLEN ──────────────────────
const GRAPH_X_RANGE = 7;   // hoeveel x-eenheden links/rechts zichtbaar
const GRAPH_Y_RANGE = 10;   // hoeveel y-eenheden boven/onder zichtbaar
// ─────────────────────────────────────────────────
...
<li>Pak de draaiende diamantjes om 3 seconden lang dwars door de hindernissen te kunnen lopen</li>
    ...
    closeBtn.addEventListener('touchend', function(e) { e.preventDefault(); popup.remove(); overlay.remove(); });
    overlay.addEventListener('click', function(e) { if (e.target === overlay) { popup.remove(); overlay.remove(); } });
    overlay.addEventListener('touchend', function(e) { if (e.target === overlay) { e.preventDefault(); popup.remove(); overlay.remove(); } });
}
...
function showScoreFeedback() {
    ...
    let popup = document.createElement('div');
    popup.id = 'scorePopup';
    popup.style.cssText = `position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background-color:white;border:3px solid ${feedbackColor};border-radius:10px;padding:30px;max-width:500px;box-shadow:0 4px 20px rgba(0,0,0,0.3);z-index:1000;font-family:Arial,sans-serif;`;
    ...
    popup.innerHTML = `
        ...
        <button id="closeFeedbackBtn" style="background-color:${feedbackColor};color:white;border:none;padding:10px 30px;border-radius:5px;cursor:pointer;font-size:16px;margin-top:20px;">Sluiten</button>
    `;
...
// =====================================================
//  VRAGEN GENEREREN — machtsfuncties VWO klas 3
// =====================================================
function generateQuestions() {
    ...
    // Functies zonder haakjes: y = ax^n + c  (p = 0)
    // Functies met haakjes:    y = a(x-p)^n + c
    // Unicode: ² = \u00B2  ³ = \u00B3  ⁴ = \u2074  − = \u2212
    const allFunctions = [
        // ── Kwadratisch zonder haakjes ──
        { eq: 'y = x\u00B2',                           type: 'parabola', a:  1,   p: 0,  c:  0 },
        { eq: 'y = \u2212x\u00B2',                     type: 'parabola', a: -1,   p: 0,  c:  0 },
        { eq: 'y = 3x\u00B2',                          type: 'parabola', a:  3,   p: 0,  c:  0 },
        { eq: 'y = \u22125x\u00B2',                    type: 'parabola', a: -5,   p: 0,  c:  0 },
        { eq: 'y = x\u00B2 + 2',                       type: 'parabola', a:  1,   p: 0,  c:  2 },
        { eq: 'y = x\u00B2 \u2212 3',                  type: 'parabola', a:  1,   p: 0,  c: -3 },
        { eq: 'y = \u2212x\u00B2 + 1',                 type: 'parabola', a: -1,   p: 0,  c:  1 },
        { eq: 'y = \u2212x\u00B2 \u2212 1',            type: 'parabola', a: -1,   p: 0,  c: -1 },
        { eq: 'y = 2x\u00B2 \u2212 3',                 type: 'parabola', a:  2,   p: 0,  c: -3 },

        // ── Kwadratisch met haakjes ──
        { eq: 'y = (x \u2212 2)\u00B2',                type: 'parabola', a:  1,   p:  2, c:  0 },
        { eq: 'y = (x + 3)\u00B2',                     type: 'parabola', a:  1,   p: -3, c:  0 },
        { eq: 'y = \u2212(x \u2212 1)\u00B2',          type: 'parabola', a: -1,   p:  1, c:  0 },
        { eq: 'y = 2(x \u2212 1)\u00B2 \u2212 3',      type: 'parabola', a:  2,   p:  1, c: -3 },
        { eq: 'y = \u22122(x + 1)\u00B2 + 5',         type: 'parabola', a: -2,   p: -1, c:  5 },
        { eq: 'y = (x \u2212 2)\u00B2 + 1',            type: 'parabola', a:  1,   p:  2, c:  1 },
        { eq: 'y = (x + 3)\u00B2 \u2212 4',            type: 'parabola', a:  1,   p: -3, c: -4 },

        // ── Kubisch zonder haakjes ──
        { eq: 'y = x\u00B3',                           type: 'cubic',    a:  1,   p: 0,  c:  0 },
        { eq: 'y = \u2212x\u00B3',                     type: 'cubic',    a: -1,   p: 0,  c:  0 },
        { eq: 'y = 2x\u00B3',                          type: 'cubic',    a:  2,   p: 0,  c:  0 },
        { eq: 'y = \u22122x\u00B3',                    type: 'cubic',    a: -2,   p: 0,  c:  0 },
        { eq: 'y = 0,3x\u00B3',                        type: 'cubic',    a:  0.3, p: 0,  c:  0 },
        { eq: 'y = x\u00B3 + 2',                       type: 'cubic',    a:  1,   p: 0,  c:  2 },
        { eq: 'y = x\u00B3 \u2212 3',                  type: 'cubic',    a:  1,   p: 0,  c: -3 },
        { eq: 'y = \u2212x\u00B3 + 4',                 type: 'cubic',    a: -1,   p: 0,  c:  4 },

        // ── Kubisch met haakjes ──
        { eq: 'y = (x \u2212 2)\u00B3',                type: 'cubic',    a:  1,   p:  2, c:  0 },
        { eq: 'y = (x + 1)\u00B3',                     type: 'cubic',    a:  1,   p: -1, c:  0 },
        { eq: 'y = \u2212(x + 1)\u00B3 + 3',           type: 'cubic',    a: -1,   p: -1, c:  3 },
        { eq: 'y = 2(x \u2212 1)\u00B3 + 2',           type: 'cubic',    a:  2,   p:  1, c:  2 },
        { eq: 'y = \u2212(x \u2212 2)\u00B3 \u2212 1', type: 'cubic',    a: -1,   p:  2, c: -1 },

        // ── Vierde macht met haakjes ──
        { eq: 'y = (x \u2212 1)\u2074',                type: 'quartic',  a:  1,   p:  1, c:  0 },
        { eq: 'y = \u2212(x + 2)\u2074 + 3',           type: 'quartic',  a: -1,   p: -2, c:  3 },
        { eq: 'y = 3(x \u2212 5)\u2074 \u2212 1',      type: 'quartic',  a:  3,   p:  5, c: -1 },
        { eq: 'y = 0,5(x + 1)\u2074',                  type: 'quartic',  a:  0.5, p: -1, c:  0 },
        { eq: 'y = (x \u2212 1)\u2074 \u2212 2',       type: 'quartic',  a:  1,   p:  1, c: -2 },
    ];

    // Kies 10 willekeurige unieke functies
    let shuffled = shuffle(allFunctions.slice());
    let questions = shuffled.slice(0, 10);
    let answers = shuffle(questions.slice());

    // Maak vraagblokken (rijen 1 en 2)
    let questionIndex = 0;
    for (let row = 1; row < 3; row++) {
        for (let col = 0; col < COLS; col++) {
            blocks.push({
                ...
                isDragging: false,
                isPlaced: false,
                text: questions[questionIndex].eq,
                fnType: questions[questionIndex].type,
                fnA: questions[questionIndex].a,
                fnP: questions[questionIndex].p,
                fnC: questions[questionIndex].c,
                matchId: questionIndex,
                isQuestion: true,
                isCorrect: null,
                isHovered: false,
                hoverProgress: 0,
    ...
    // Maak antwoordblokken (rijen 3 en 4)
    let answerIndex = 0;
    for (let row = 3; row < 5; row++) {
        for (let col = 0; col < COLS; col++) {
            let fn = answers[answerIndex];
            let matchId = questions.findIndex(q =>
                q.eq === fn.eq && q.type === fn.type &&
                q.a === fn.a && q.p === fn.p && q.c === fn.c
            );
            blocks.push({
                col: col,
                row: row,
...
function setup() {
    ...
    loadImage('background_dragon.png', 
      ...
      () => { loadImage('background_dragon.png', (img) => { backgroundImage = img; bgLoaded = true; }); }
    );
    ...
    canvasButtons = [
      ...
      new CanvasButton(startX + (btnW + btnGap) * 3 + 20, btnY, btnW, btnH, '\u24d8 Info', color(3, 169, 244), showInfo)
    ];
...
function draw() {
    ...
    for (let block of blocks) {
        ...
        if (!IS_MOBILE && !showDinoGame && block.isQuestion &&
            block.row >= 1 && block.row < 3 && !draggingBlock &&
            mouseX >= block.x && mouseX <= block.x + CELL_SIZE &&
            mouseY >= block.y && mouseY <= block.y + CELL_SIZE) {
            block.isHovered = true;
        }
        block.hoverProgress = lerp(block.hoverProgress, block.isHovered ? 1 : 0, 0.15);
    }
    ...
    for (let btn of canvasButtons) {
        if (!IS_MOBILE && !draggingBlock) { btn.checkHover(mouseX, mouseY); }
        btn.draw();
    }
    ...
    setCursor(IS_MOBILE ? 'default' : (showHandCursor ? 'pointer' : 'default'));
}
...
function drawBlock(block) {
    ...
    if (block.isQuestion) {
        ...
        // Antwoordblok: teken mini-grafiek
        drawMiniGraph(
            bx + 10, by + 10,
            CELL_SIZE - 20, CELL_SIZE - 20,
            block.fnType, block.fnA, block.fnP, block.fnC
        );
    }
...
// =====================================================
//  MINI-GRAFIEK TEKENEN
//  - assen altijd vast door oorsprong (cx, cy)
//  - labels x en y aan het eind van de assen
//  - geen pijltjes
//  - p en c alleen in de wiskundige formule
// =====================================================
function drawMiniGraph(gx, gy, gw, gh, type, a, p, c) {
    let cx = gx + gw / 2;   // y-as: altijd midden van blokje
    let cy = gy + gh / 2;   // x-as: altijd midden van blokje
    p = p || 0;
    c = c || 0;

    // ── BEREIK INSTELLEN ─────────────────────────────
    let xRange = GRAPH_X_RANGE;   // pas bovenaan het bestand aan
    let yRange = GRAPH_Y_RANGE;
    // ─────────────────────────────────────────────────

    let scaleX = (gw - 4) / (2 * xRange);
    let scaleY = (gh - 4) / (2 * yRange);

    // Vaste assen door de oorsprong (cx, cy)
    stroke(30);
    strokeWeight(1.2);
    line(gx + 4, cy, gx + gw - 4, cy);   // x-as
    line(cx, gy + 4, cx, gy + gh - 4);   // y-as

    // As-labels (geen pijltjes)
    fill(30); noStroke();
    textSize(9); textStyle(BOLD);
    textAlign(LEFT, CENTER);
    text('x', gx + gw - 3, cy - 6);      // x rechts van x-as
    textAlign(CENTER, BOTTOM);
    text('y', cx + 5, gy + 10);           // y boven y-as
    ...
    // Grafiekkromme — p en c zitten alleen in de formule
    stroke(20, 40, 160);
    strokeWeight(2);
    noFill();
    beginShape();
    for (let px = gx + 2; px <= gx + gw - 2; px++) {
        let x = (px - cx) / scaleX;      // x relatief aan vaste oorsprong
        let dx = x - p;                   // horizontale verschuiving
        let y;
        if (type === 'cubic')        y = a * dx * dx * dx + c;
        else if (type === 'quartic') y = a * dx * dx * dx * dx + c;
        else                         y = a * dx * dx + c;   // parabola
        let py = cy - y * scaleY;         // y relatief aan vaste oorsprong
        if (py < gy + 2 || py > gy + gh - 2) {
            endShape();
            beginShape();
            continue;
        }
...
function checkAnswers() {
    ...
    for (let questionBlock of blocks) {
        if (questionBlock.isQuestion) {
            ...
            for (let block of blocks) {
                if (!block.isQuestion &&
                    block.col === questionBlock.col &&
                    block.row === questionBlock.row) {
                    answerBlock = block;
                    break;
                }
            }
            if (answerBlock && questionBlock.matchId === answerBlock.matchId) {
    ...
    if (canvasButtons.length > 1) {
        canvasButtons[1].color = correctCount === 10
            ? color(255, 215, 0)
            : color(156, 39, 176);
    }
    if (correctCount === 10) {
